...
public class CrosshatchStandardFoliageGUI : ShaderGUI {
    ...
    enum AnimationMode
    {
        None, Swaying, Swelling, VertexColor, Glitchy, Circle
    }
    ...
    {
        AnimationMode am = AnimationMode.Swaying;

        if (IsKeywordEnabled("NO_ANIMATION"))
        {
            am = AnimationMode.None;
        }
        if (IsKeywordEnabled("W_POS_SWELLING"))
        {
            am = AnimationMode.Swelling;
        }
        if (IsKeywordEnabled("W_POS_SWAYING"))
        {
            am = AnimationMode.Swaying;
        }
        if (IsKeywordEnabled("V_COLOR_SWAYING"))
        {
            am = AnimationMode.VertexColor;
        }
        if (IsKeywordEnabled("V_COLOR_GLITCHY"))
        {
            am = AnimationMode.Glitchy;
        }
        if (IsKeywordEnabled("V_COLOR_CIRCLE"))
        {
            am = AnimationMode.Circle;
        }

        EditorGUI.BeginChangeCheck();
        am = (AnimationMode)EditorGUILayout.EnumPopup("Animation Mode", am);
        if (EditorGUI.EndChangeCheck())
        {
            editor.RegisterPropertyChangeUndo("Animation Mode");
            SetKeyword("NO_ANIMATION", am == AnimationMode.None);
            SetKeyword("W_POS_SWELLING", am == AnimationMode.Swelling);
            SetKeyword("W_POS_SWAYING", am == AnimationMode.Swaying);
            SetKeyword("V_COLOR_SWAYING", am == AnimationMode.VertexColor);
            SetKeyword("V_COLOR_GLITCHY", am == AnimationMode.Glitchy);
            SetKeyword("V_COLOR_CIRCLE", am == AnimationMode.Circle);
        }
    }
    ...
    {
        GUILayout.Label("Vertex Animation", EditorStyles.boldLabel);
        SetAnimationMode();
        FloatProperty("_AnimationMagnitude", "");
        FloatProperty("_AnimationSpeed", "");
        FloatProperty("_AnimationWaveSize", "");

        if(IsKeywordEnabled("W_POS_SWAYING") || IsKeywordEnabled("V_COLOR_SWAYING") || IsKeywordEnabled("V_COLOR_GLITCHY"))
        {
            VectorProperty("_AnimationSway", "");
        }

        if (IsKeywordEnabled("V_COLOR_GLITCHY"))
        {
            VectorProperty("_AnimationTimeStep", "");
        }
    ...
    void VectorProperty(string prop, string tip)
    {
        MaterialProperty vec = FindProperty(prop);
        GUIContent vecLabel = new GUIContent(vec.displayName, tip);
        editor.ShaderProperty(vec, vecLabel);
    }
