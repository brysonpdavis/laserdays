...
inline fixed3 GaussianBlur5x5(fixed3 textures[25])
{
	...
	return ApplyKernel5x5(KERNEL_GBLUR5X5, textures);
       ...
       inline fixed3 HSVToRGB(fixed3 c )
        {
       ...
       inline fixed3 RGBToHSV(fixed3 c)
        {
        ...
        inline fixed3 RGBToHUE(float3 c)
        {
            float4 K = float4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);
            float4 p = lerp( float4( c.bg, K.wz ), float4( c.gb, K.xy ), step( c.b, c.g ) );
            float4 q = lerp( float4( p.xyw, c.r ), float4( c.r, p.yzx ), step( p.x, c.r ) );
            float d = q.x - min( q.w, q.y );
            float e = 1.0e-10;
            float ff = abs(q.z + (q.w - q.y) / (6.0 * d + e));
...
inline fixed4 ApplySobel(fixed3 textures[9])
{
	...
	for (int i = 0, k = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++, k++)
		{
			float Gx = KERNEL_SOBELX[i][j] * RGBToHUE(textures[k]);
			float Gy = KERNEL_SOBELY[i][j] * RGBToHUE(textures[k]);
